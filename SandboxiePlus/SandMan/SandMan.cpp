	...
	: QMainWindow(parent)
{
...
#ifdef INSIDER_BUILD
	QString appTitle = tr("Sandboxie-Plus (nuiziyyds Modified) Insider [%1]").arg(QString(__DATE__));
#else
	QString appTitle = tr("Sandboxie-Plus (nuiziyyds Modified) v%1").arg(GetVersion());
#endif
	...
	m_pProgressDialog->setWindowTitle("Sandboxie-Plus (nuiziyyds Modified)");
...
void CSandMan::CreateHelpMenu(bool bAdvanced)
{
	m_pMenuHelp = m_pMenuBar->addMenu(tr("&Help"));
		...
		m_pAbout = m_pMenuHelp->addAction(CSandMan::GetIcon("IconFull", 2), tr("About Sandboxie-Plus (nuiziyyds Modified)"), this, SLOT(OnAbout()));
}
	...
	if (bConnected)
	{
		...
		if (theAPI->GetSecureParam("UsageFlags", &UsageFlags, sizeof(UsageFlags))) {
			if (!CERT_IS_TYPE(g_CertInfo, eCertBusiness)) {
				if ((UsageFlags & (2 | 1)) != 0) {
					appTitle.append(tr(" - nuiziyyds Modified"));
				}
			}
		}
...
void CSandMan::ShowMessageBox(QWidget* Widget, QMessageBox::Icon Icon, const QString& Message)
{
	...
	msgBox.setIcon(Icon);
	msgBox.setWindowTitle("Sandboxie-Plus (nuiziyyds Modified)");
...
{
    QMessageBox mb(this);
    mb.setWindowTitle("Sandboxie-Plus (nuiziyyds Modified)");
...
void CSandMan::OnAbout()
{
	if (sender() == m_pAbout)
	{
		...
		QString AboutCaption = tr(
			"<h3>About Sandboxie-Plus (nuiziyyds Modified)</h3>"
			"<p>Version %1</p>"
			"<p>" MY_COPYRIGHT_STRING "</p>"
			"<p>Modified by nuiziyyds - For personal use only</p>"
		).arg(theGUI->GetVersion(true));

		QString CertInfo;
		CertInfo = tr("nuiziyyds Modified - For personal use only");
		...
		QMessageBox *msgBox = new QMessageBox(this);
		msgBox->setAttribute(Qt::WA_DeleteOnClose);
		msgBox->setWindowTitle(tr("About Sandboxie-Plus (nuiziyyds Modified)"));
